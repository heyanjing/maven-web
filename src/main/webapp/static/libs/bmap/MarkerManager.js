...
(function() {
	 ...
	 * MarkerManger类的构造函数
	 * 
	 * @class MarkerManager <b>入口</b>。 实例化该类后，可调用addMarkers,show,hide等方法，控制marker。 请注意，此类比较适用于以下情况： 大量marker分布到不同的zoom级别中，比如：18级的时候显示100个marker，15级的时候显示另外的100个marker
	 * @constructor
	 * @param {Map} map Baidu map的实例对象.
	 * @param {Json Object} opts 可选的输入参数，非必填项。可输入选项包括：<br /> {<br />"<b>borderPadding</b>" : {Number} 当前viewport额外的padding，落在这个padding中的marker会被添加到地图上，即使他们不是完全可见的。 <br />"<b>maxZoom</b>" : {Number} 设置需要marker manager需要监视的最大的zoom，如果没有给出，默认为地图的最大的zoom。. <br /> }<br />.
	 * @example <b>参考示例：</b><br />
	...
	MarkerManager.prototype.addMarkers = function(markers, minZoom, maxZoom) {
		var len = markers.length, me = this;
		for (var i = len; i--;) {
	...
	MarkerManager.prototype.getMarkerCount = function(zoom) {
		var len = this._numMarkers.length, t = this._numMarkers, count = 0;
		for (var i = len; i--;) {
	...
	baidu.object.extend(MarkerManager.prototype, {
		_showMarkers : function() {
			var num = this._numMarkers.length, curZoom = this._map.getZoom(), t = this._numMarkers, curBounds = this._getRealBounds();
			for (var i = num; i--;) {
				var title = t[i].z.title, isIn = curBounds.containsPoint(t[i].getPosition()), msg = title + '----------' + isIn;
				// console.log(msg);
				...
				if (curBounds.containsPoint(t[i].getPosition()) && curZoom >= t[i].minZoom && curZoom <= t[i].maxZoom) {
					...
					if (!t[i].bAdded) {
						...
						// 显示marker
						this._visible && t[i].show();// XXX 九龙坡的点已经不在地图上了
					}
				} else if (t[i].bAdded) {
		...
		_getRealBounds : function() {
			var curBounds = this._map.getBounds(), southWest = this._map.pointToPixel(curBounds.getSouthWest()), northEast = this._map.pointToPixel(curBounds.getNorthEast()), extendSW = {
			...
			}, extendSwPoint = this._map.pixelToPoint(extendSW), extendNePoint = this._map.pixelToPoint(extendNE);
		...
		_removeMarkerFromArray : function(marker) {
			var num = this._numMarkers.length, i = num, shift = 0;
			for (i = num; i--;) {
